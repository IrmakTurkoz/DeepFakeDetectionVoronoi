...
def crop_image(polygon, img):
    ...
    if black/ (dst.shape[0]*dst.shape[1] )<0.4  and dst.shape[0] > 20 and dst.shape[1] > 20: 
        ...
        cv2.imwrite("features/"+ str(COUNT) + ".jpg", dst)
        increment()
    return dst

def createVoronoi(img,predictor,rect,face_id,show_results = False):
    ...
    for convex_vertices in vor.regions:
        if all(v > 0 for v in convex_vertices):
            ...
            if polygon:
                mask = crop_image(polygon,orginal_cutted_img)
                masks.append(mask)
